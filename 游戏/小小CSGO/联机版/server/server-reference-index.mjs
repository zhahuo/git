...
if (!Number.isInteger(port) || port < 1 || port > 65535 || typeof host !== 'string' || host.length === 0) {
  console.error('STRIKE//ZONE invalid GAME_PORT or GAME_HOST configuration');
  process.exit(1);
}
httpServer.listen(port, host, () => console.log(`STRIKE//ZONE game server listening on ws://${host}:${port}`));
...
if (stateDir) {
  loadState(stateDir, 'ranking').then((data) => {
    if (data && restoreRanking(rankingStore, data)) {
      ...
      console.log('STRIKE//ZONE restored ranking state');
    }
  }).catch(() => {});
...
function beginGracefulShutdown(source) {
  ...
  console.log(`STRIKE//ZONE graceful shutdown started (${source})`);
